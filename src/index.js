...
var node_global_key_listener_1 = require("node-global-key-listener");
...
var windows = node_screenshots_1.Window.all();
...
keyboard.addListener(function (event) {
    ...
    if (event.name === 'RIGHT CTRL' && event.state === 'DOWN') { //右コントロール　スクショ
        (0, fs_1.readFileSync)(".secret_targetWindows", { encoding: "utf-8" }).split("\r\n").forEach(function (tg_window, i, a) {
            windows.forEach(function (item) {
                if (item.appName == tg_window) {
                    var image = item.captureImageSync();
                    var filename = "pix/".concat(item.appName, "_").concat(date.toLocaleString().replace(/\//g, "_").replace(/:/g, "_"), ".png");
...
setInterval(function () {
    ...
    (0, fs_1.readFileSync)(".secret_targetAutoWindows", { encoding: "utf-8" }).split("\r\n").forEach(function (tg_window, i, a) {
